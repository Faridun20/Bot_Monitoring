...
**честной ротацией, не чистой случайностью**. Все активные посты один раз
перемешиваются в случайном порядке и уходят по очереди, по одному за слот;
когда очередь заканчивается — перемешивается заново. Гарантия: за N слотов
подряд (N — число активных постов) каждый пост уйдёт ровно один раз, и ни
один пост не повторится сразу за собой — ни внутри цикла, ни на стыке между
перемешиваниями. Если снять `#active` с поста, который ещё стоит в очереди —
он просто тихо выпадает из неё при следующей рассылке.
